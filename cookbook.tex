...
\begin{itemize}
  ...
  \item \T{Probability Measure}
    \begin{enumerate}
\item $\mathbb{P}(A)\geq 0\: \forall \: A\in\mathcal{B}$
\item $\mathbb{P}$ is $\sigma$-additive for disjoint Events $A_{n}$
\[
\mathbb{P} \left( \bigcup\limits_{n=1}^{\infty} A_{n} \right)= \Sum[n=1]{\infty}\mathbb{P}(A_{n}).
\]
...
\textsc{H\"older}\label{hoelder} \\
\[
p>1,\; q>1,\; \frac{1}{p}+\frac{1}{q}=1
\]
and that
\[
\E{\Abs{X}^{p}}<\infty,\;\E{\Abs{X}^{q}}<\infty
\]
then
\[
\Abs{\E{XY}}\leq \E{\Abs{XY}} \leq \left(\E{\Abs{X}^{p}}\right)^{1/p}\left(\E{\Abs{Y}^{q}}\right)^{1/q}
\]
